...
**하나의 앱**으로 개발한다. 앱을 두 개로 나누지 않고, 로그인한 계정의 역할(role)에 따라 화면·기능·API 접근을 분기한다.

- **유저(User) = 응급구조대원(구급대원)**: 병원 조회/추천 확인, 환자 상태 음성 입력, 수용 요청 전송, 요청 상태 확인, 수락 병원 길안내.
- **어드민(Admin) = 병원 측(병원 담당자/응급실)**: 들어온 요청 목록/상세 확인, 환자 수용 **수락**, 요청 알림 관리.

로그인 응답의 역할값으로 최초 진입 화면(유저 플로우 / 어드민 플로우)을 결정한다.
...
- **앱 진입/역할 분기**: 단일 앱. 루트 `View`가 로그인 상태와 역할을 보고 유저(구급대원) 플로우 / 어드민(병원) 플로우로 분기한다. 기능 폴더도 `User`(구급대원)와 `Admin`(병원)으로 나누고, 공통은 `Shared`/`Core`에 둔다.
- **네트워킹**: `async/await` + `URLSession` 기반 서비스 계층. 백엔드는 REST + Swagger(OpenAPI) 명세 제공 예정 → DTO는 명세에 맞춰 `Codable`로 정의.
- **인증**: access token은 Keychain 저장. 로그인 응답의 역할값으로 라우팅하고, 역할 기반 접근 제어(RBAC)로 유저/어드민 API 호출을 분리한다.
...
### 빌드 / 실행 검증
- **빌드·실행 검증은 항상 사용자가 한다.** 에이전트는 `xcodebuild`·시뮬레이터·앱 실행 등 빌드/런 검증을 직접 돌리지 않는다.
- 코드 변경 후에는 무엇을 어떻게 확인하면 되는지만 안내하고, 실제 빌드/실행은 사용자에게 맡긴다.
